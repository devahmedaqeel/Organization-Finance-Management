...
export function buildPayslipHtml(
  ...
  const slipId = "SLIP-" + (payslip.id || "0000").slice(-6).toUpperCase() + "-" + Date.now().toString(36).toUpperCase();
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; color: #0F172A; background: #FFFFFF; font-size: 11px; }
    @media print { body { background: #FFFFFF; } .no-print { display: none !important; } }
    @media screen { body { max-width: 800px; margin: 0 auto; padding: 24px 16px; background: #F1F5F9; } .card { background: #FFF; padding: 24px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); } }
    .header { background: linear-gradient(135deg, #1E1B4B 0%, #312E81 50%, #4338CA 100%); color: #FFF; padding: 18px 20px; border-radius: 10px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center; }
    .org-title { font-size: 18px; font-weight: 900; }
    .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; background: #F8FAFC; border: 1px solid #E2E8F0; padding: 14px; border-radius: 8px; margin-bottom: 16px; }
    .info-lbl { font-size: 9.5px; font-weight: 700; color: #64748B; text-transform: uppercase; }
    .info-val { font-weight: 700; color: #0F172A; margin-top: 2px; }
    table { width: 100%; border-collapse: collapse; margin-bottom: 16px; }
    th { background: #F1F5F9; font-size: 10px; font-weight: 700; color: #475569; text-align: left; padding: 8px 10px; }
    td { padding: 9px 10px; border-top: 1px solid #F1F5F9; font-size: 11px; }
    .text-right { text-align: right; }
    .net-card { background: linear-gradient(135deg, #7C3AED 0%, #6D28D9 100%); color: #FFF; padding: 16px 18px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
    .sign-row { display: flex; justify-content: space-between; margin-top: 36px; padding-top: 12px; }
    .sign-box { width: 180px; text-align: center; border-top: 1px dashed #94A3B8; padding-top: 6px; font-size: 10px; color: #64748B; }
  </style>
</head>
<body>
  <div class="no-print" style="position:fixed; top:12px; right:16px; z-index:9999;">
    <button onclick="window.print()" style="background:#7C3AED; color:#FFF; font-weight:700; border:none; padding:8px 16px; border-radius:6px; cursor:pointer;">🖨️ Print / Save as PDF</button>
  </div>
  <div class="card">
    <div class="header">
      <div>
        <div class="org-title">${orgName}</div>
        <div style="font-size: 10.5px; color: #C7D2FE; margin-top: 2px;">${orgAddress} · ${orgEmail} · ${orgPhone}</div>
      </div>
      <div style="text-align: right;">
        <div style="font-size: 12px; font-weight: 800; color: #67E8F9;">OFFICIAL PAYSLIP</div>
        <div style="font-size: 10px; color: #E0E7FF;">Period: <strong>${payslip.month || "Current"}</strong></div>
        <div style="display:inline-block; background:#10B981; color:#FFF; font-weight:800; font-size:9px; padding:2px 6px; border-radius:4px; margin-top:4px;">✓ DISBURSED</div>
      </div>
    </div>
    ...
    <div class="info-grid">
      ...
      <div><div class="info-lbl">Department</div><div class="info-val">${payslip.department || "General"}</div></div>
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>Basic Salary</td>
          <td class="text-right" style="font-weight:700;">+${currency} ${fmt(base)}</td>
          <td>Income Tax Withholding</td>
          <td class="text-right" style="color:#F43F5E; font-weight:700;">-${currency} ${fmt(ded * 0.6)}</td>
        </tr>
        <tr>
          <td>Bonuses & Performance Allowances</td>
          <td class="text-right" style="color:#10B981; font-weight:700;">+${currency} ${fmt(bon)}</td>
          <td>Provident & Health Deductions</td>
          <td class="text-right" style="color:#F43F5E; font-weight:700;">-${currency} ${fmt(ded * 0.4)}</td>
        </tr>
        <tr style="background:#F8FAFC; font-weight:800;">
          <td>Total Gross Earnings</td>
          <td class="text-right" style="color:#10B981;">+${currency} ${fmt(base + bon)}</td>
          <td>Total Deductions</td>
          <td class="text-right" style="color:#F43F5E;">-${currency} ${fmt(ded)}</td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="net-card">
      <div>
        <div style="font-size:11px; font-weight:800; letter-spacing:0.5px;">NET SALARY PAYABLE & DISBURSED</div>
        <div style="font-size:9.5px; opacity:0.9; margin-top:2px;">Direct institutional bank deposit verified. Ref: ${slipId}</div>
      </div>
      <div style="font-size:22px; font-weight:900;">${currency} ${fmt(net)}</div>
    </div>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    body {
      ...
      color: #0F172A; background: #FFFFFF; font-size: 10px; line-height: 1.35;
    ...
    @media screen {
      body { max-width: 880px; margin: 0 auto; padding: 18px 14px 40px 14px; background: #F1F5F9; }
      .sheet-wrap { background: #FFF; padding: 22px; border-radius: 10px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); }
    }
    ...
    .kpi-val { font-size: 14.5px; font-weight: 800; }
    ...
    thead th {
      ...
      padding: 6px 8px; border-bottom: 1.5px solid #CBD5E1; text-align: left;
    }
...
<body>
  ...
  <div class="header-card">
    <div style="display:flex; align-items:center; gap:14px; flex:1.4;">
      ...
      <div>
        ...
        <div class="org-contact">${metadata.organizationAddress} · ${metadata.organizationEmail} · ${metadata.organizationPhone}</div>
      </div>
    </div>
    <div class="meta-box">
  ...
  ${monthlyTrends.chartPoints.length > 0 ? `
  ...
  <table style="table-layout: fixed; width: 100%;">
    <thead>
      <tr>
        <th style="width: 26%;">Department Name</th>
    ...
    <tfoot>
      <tr class="tfoot-row">
        ...
        <td class="num">${currency} ${fmt(budgetPerformance.totalAllocated)}</td>
        <td class="num" style="color: #F43F5E;">${currency} ${fmt(budgetPerformance.totalSpent)}</td>
        <td class="num" style="color: ${budgetPerformance.totalRemaining >= 0 ? "#10B981" : "#F43F5E"};">${currency} ${fmt(budgetPerformance.totalRemaining)}</td>
  ...
  <table style="table-layout: fixed; width: 100%;">
    <thead>
      <tr>
        ...
        <th style="width: 14%; text-align: right;">Base Salary</th>
        <th style="width: 10%; text-align: right;">Bonus</th>
        <th style="width: 10%; text-align: right;">Deductions</th>
        <th style="width: 12%; text-align: right;">Net Disbursed</th>
      </tr>
    </thead>
    <tbody>
