...
export function StatCard({ label, value, icon, color, subtitle, flex = 1 }: StatCardProps) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border, flex }]}>
      ...
      <Text style={[styles.value, { color: colors.foreground }]} numberOfLines={2} adjustsFontSizeToFit minimumFontScale={0.65}>{value}</Text>
      {subtitle ? <Text style={[styles.subtitle, { color: accentColor }]} numberOfLines={2}>{subtitle}</Text> : null}
    </View>
  );
}
